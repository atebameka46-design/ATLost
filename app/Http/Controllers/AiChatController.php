    ...
    public function message(Request $request): View
    {
        ...
        try {
            $response = Http::withHeaders(['x-goog-api-key' => config('services.gemini.key')])->post('https://generativelanguage.googleapis.com/v1beta/models/'.config('services.gemini.model', 'gemini-3.8-flash').':generateContent', [
                'system_instruction' => ['parts' => [['text' => 'Tu es ATBot, l’assistant ATLost. Réponds en français, simplement et brièvement. Aide sur la recherche, le signalement et la récupération de documents au Cameroun. Ne demande jamais de mot de passe ni de paiement par chat.']]],
            ...
            $message = $exception->response?->status() === 403 ? 'ATBot est connecté, mais votre projet Google n’a pas accès à ce modèle ou a dépassé ses limites.' : 'ATBot est momentanément indisponible. Vérifiez votre clé API et réessayez.';
